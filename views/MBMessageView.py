...
async def send_message(request: Request):
    ...
    try:
        user_from = await MBUser.find_one({'id': ObjectId(request.args.get("from_user_id"))})
    except InvalidId:
        return MBRequest.invalid_user_id(request.args.get("from_user_id"))
...
@doc.consumes(
    doc.String(name="user_id", description="User id", required=True)
)
@message_api.get("/")
async def get_massages(request: Request):
    ...
    user: MBUser
    try:
        user = await MBUser.find_one({'id': ObjectId(request.args.get("user_id"))})
    except InvalidId:
        return MBRequest.invalid_user_id(request.args.get("user_id"))
